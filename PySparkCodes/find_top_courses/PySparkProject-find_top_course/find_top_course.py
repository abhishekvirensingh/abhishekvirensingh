...
map_chapters_data = chapters.map(lambda x: (str(x.split(",")[1]), 1))
count_of_chapters_rdd = map_chapters_data.reduceByKey(lambda x, y: x + y)
count_of_chapters_temp = count_of_chapters_rdd.map(lambda x: (x[0], x[1]))
count_of_chapters_sorted = count_of_chapters_temp.sortByKey()
count_of_chapters = count_of_chapters_sorted.collect()
for aline in count_of_chapters[0:5]:
    print(aline)
...
def get_score_data(course_id_percentage):
    ...
    else: return str(course_id_percentage[0]), 0.0
...
chapter_id_user_id_course_id = user_id_chap_id.map(lambda x: (x[0], x[1][0], x[1][1]))
